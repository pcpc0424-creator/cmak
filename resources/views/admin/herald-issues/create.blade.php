...
@section('title', 'CM Herald 호 등록')

@section('content')
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">CM Herald 호 등록</h1>
        <p class="mt-1 text-sm text-gray-500">표지 썸네일과 웹진보기 링크(또는 PDF)를 등록합니다.</p>
    </div>

    <form action="{{ url('/admin/herald-issues') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="bg-white rounded-lg shadow p-6 space-y-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">제목(호수) <span class="text-red-500">*</span></label>
                <input type="text" name="title" value="{{ old('title') }}" required placeholder="예: 2026년 6월호 / 제120호"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="issue_date" value="{{ old('issue_date') }}" class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">정렬순서</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order') }}" min="0" placeholder="자동" class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">표지 이미지</label>
                ...
                <p class="mt-1 text-xs text-gray-400">세로형 표지 권장(3:4 비율). JPG, PNG</p>
            ...
            <div class="border-t pt-5">
                ...
                <input type="text" name="webzine_url" value="{{ old('webzine_url') }}" placeholder="https://... (외부 e-book) 또는 비워두고 아래 PDF 업로드"
                       class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                ...
                <input type="file" name="webzine_file" accept="application/pdf"
                       class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer">
                <p class="mt-1 text-xs text-gray-400">PDF를 올리면 '웹진보기'가 이 PDF로 연결됩니다. (URL보다 우선)</p>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full rounded-md border-gray-300 shadow-sm text-sm">{{ old('description') }}</textarea>
            </div>

            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_published" value="1" {{ old('is_published', '1') ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 shadow-sm focus:ring-blue-500">
