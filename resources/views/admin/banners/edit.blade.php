...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    {{-- 헤더 --}}
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">배너 수정</h1>
        <p class="mt-1 text-sm text-gray-500">{{ $banner->title }} 배너를 수정합니다.</p>
    </div>
    ...
    <form action="{{ url('/admin/banners/' . $banner->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-6">
            ...
            <div>
                ...
                @if($banner->image_path && file_exists(public_path($banner->image_path)))
                    <div class="mb-3">
                        <img src="/cmak/{{ $banner->image_path }}" alt="{{ $banner->title }}"
                             class="max-w-xs h-auto object-contain border border-gray-200 rounded p-1 bg-white">
                        <p class="mt-1 text-xs text-gray-500">현재 이미지</p>
                    </div>
                @elseif($banner->image_path)
                    <div class="mb-3">
                        <p class="text-xs text-red-500">이미지 파일 없음: {{ $banner->image_path }}</p>
                    </div>
                @endif
                ...
                <p class="mt-1 text-xs text-gray-400">새 파일 업로드 시 기존 이미지가 교체됩니다.</p>
                @error('image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="started_at" id="started_at"
                           value="{{ old('started_at', $banner->started_at ? \Carbon\Carbon::parse($banner->started_at)->format('Y-m-d\TH:i') : '') }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                    @error('started_at')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="datetime-local" name="ended_at" id="ended_at"
                           value="{{ old('ended_at', $banner->ended_at ? \Carbon\Carbon::parse($banner->ended_at)->format('Y-m-d\TH:i') : '') }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                    @error('ended_at')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            {{-- 클릭수 (읽기 전용) --}}
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">클릭수</label>
