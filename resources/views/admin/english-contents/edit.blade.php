...
@section('title', '영문 콘텐츠 수정')

@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    {{-- 헤더 --}}
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">영문 콘텐츠 수정</h1>
        <p class="mt-1 text-sm text-gray-500">콘텐츠 #{{ $content->id }}을 수정합니다.</p>
    </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-6">
            ...
            <div>
                ...
                <select name="section" id="section"
                    ...
                    <option value="members" {{ old('section', $content->section) === 'members' ? 'selected' : '' }}>Members</option>
                    <option value="services" {{ old('section', $content->section) === 'services' ? 'selected' : '' }}>Services</option>
                    <option value="contact" {{ old('section', $content->section) === 'contact' ? 'selected' : '' }}>Contact</option>
                    <option value="news" {{ old('section', $content->section) === 'news' ? 'selected' : '' }}>News</option>
                </select>
                @error('section')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="20" required
                          class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
                          placeholder="Content">{{ old('content', $content->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="px-3 py-2 bg-gray-50 rounded-md border border-gray-200 text-sm text-gray-600 w-32">
                    {{ number_format($content->view_count ?? 0) }}
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $content->sort_order) }}" min="0"
