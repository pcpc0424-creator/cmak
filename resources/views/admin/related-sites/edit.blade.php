...
@section('title', '관련사이트 수정')

@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    {{-- 헤더 --}}
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">관련사이트 수정</h1>
        <p class="mt-1 text-sm text-gray-500">사이트 #{{ $site->id }}을 수정합니다.</p>
    </div>

    {{-- 폼 --}}
    <form action="{{ url('/admin/related-sites/' . $site->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="site_name" id="site_name" value="{{ old('site_name', $site->site_name) }}" required
            ...
            <div>
                ...
                <input type="url" name="site_url" id="site_url" value="{{ old('site_url', $site->site_url) }}" required
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
            ...
            <div>
                ...
                @if($site->logo_path)
                    <div class="mb-2 flex items-center gap-3">
                        <img src="{{ $site->logo_path }}" alt="현재 로고" class="h-10 object-contain border border-gray-200 rounded px-2 py-1 bg-gray-50">
                        <span class="text-xs text-gray-400">현재 로고</span>
                    </div>
                @endif
                ...
                <p class="mt-1 text-xs text-gray-400">새 로고를 업로드하면 기존 로고가 교체됩니다.</p>
                @error('logo')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $site->sort_order) }}" min="0"
