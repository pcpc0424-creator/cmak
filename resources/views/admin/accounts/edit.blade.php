...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    {{-- 헤더 --}}
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">계정 수정</h1>
        <p class="mt-1 text-sm text-gray-500">계정 #{{ $account->id }}을 수정합니다.</p>
    </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-400">비워두면 기존 비밀번호가 유지됩니다.</p>
                    @error('password')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <select name="role" id="role"
                        class="w-full sm:w-1/2 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                    <option value="admin" {{ old('role', $account->role) === 'admin' ? 'selected' : '' }}>관리자</option>
                    <option value="editor" {{ old('role', $account->role) === 'editor' ? 'selected' : '' }}>편집자</option>
                    <option value="viewer" {{ old('role', $account->role) === 'viewer' ? 'selected' : '' }}>뷰어</option>
                </select>
                @error('role')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="department" id="department" value="{{ old('department', $account->department) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
                           placeholder="부서명">
                    @error('department')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" name="position" id="position" value="{{ old('position', $account->position) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
                           placeholder="직위">
                    @error('position')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            {{-- 마지막 로그인 (읽기 전용) --}}
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">마지막 로그인</label>
                <div class="px-3 py-2 bg-gray-50 rounded-md border border-gray-200 text-sm text-gray-600 w-fit">
                    {{ $account->last_login_at ? $account->last_login_at->format('Y-m-d H:i:s') : '로그인 기록 없음' }}
                </div>
            </div>
