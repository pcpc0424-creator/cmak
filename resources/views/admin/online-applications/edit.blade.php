...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    {{-- 헤더 --}}
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">과정 수정</h1>
        <p class="mt-1 text-sm text-gray-500">과정 #{{ $application->id }}을 수정합니다.</p>
    </div>

    {{-- 폼 --}}
    <form action="{{ url('/admin/online-applications/' . $application->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="course_name" id="course_name" value="{{ old('course_name', $application->course_name) }}" required
            ...
            <div>
                ...
                <input type="date" name="registration_date" id="registration_date" value="{{ old('registration_date', $application->registration_date) }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="education_start" id="education_start" value="{{ old('education_start', $application->education_start) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                ...
                <div>
                    ...
                    <input type="date" name="education_end" id="education_end" value="{{ old('education_end', $application->education_end) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                    @error('education_end')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number" name="education_hours" id="education_hours" value="{{ old('education_hours', $application->education_hours) }}" min="0"
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number" name="fee" id="fee" value="{{ old('fee', $application->fee) }}" min="0"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="status" id="status"
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                        <option value="open" {{ old('status', $application->status) === 'open' ? 'selected' : '' }}>접수중</option>
                        <option value="closed" {{ old('status', $application->status) === 'closed' ? 'selected' : '' }}>마감</option>
                        <option value="completed" {{ old('status', $application->status) === 'completed' ? 'selected' : '' }}>완료</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number" name="max_participants" id="max_participants" value="{{ old('max_participants', $application->max_participants) }}" min="0"
                               class="w-32 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5"
                          class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
                          placeholder="과정에 대한 설명을 입력하세요">{{ old('description', $application->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
